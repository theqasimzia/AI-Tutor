...
import { Play, Star, Clock, ChevronRight, Zap, Target, BookOpen, Brain, Calculator, Sparkles } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Card, CardContent } from "@/components/ui/card"
import { useStudent } from "@/lib/student-context"
import { getStudentDashboardData, getSuggestedLessons } from "@/lib/queries/student"

type DashboardData = Awaited<ReturnType<typeof getStudentDashboardData>>
type SuggestedLesson = Awaited<ReturnType<typeof getSuggestedLessons>>[number]

export default function StudentDashboard() {
    const { selectedStudent, loading: ctxLoading } = useStudent()
    const [data, setData] = useState<DashboardData>(null)
    const [suggested, setSuggested] = useState<SuggestedLesson[]>([])
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            getSuggestedLessons(selectedStudent.id),
        ]).then(([d, s]) => {
            setData(d)
            setSuggested(s)
            setLoading(false)
    ...
    const isNewStudent = totalCompleted === 0 && recentLessons.length === 0
            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-6 bg-gradient-to-r from-violet-600 to-indigo-600 rounded-3xl p-8 text-white shadow-xl shadow-indigo-200 relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-2">
                    <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-md px-3 py-1 rounded-full text-xs font-medium text-white ring-1 ring-white/30">
                        {isNewStudent ? (
                            <>
                                <Sparkles className="h-3 w-3 fill-yellow-300 text-yellow-300" />
                                <span>Welcome to AI Tutor Academy!</span>
                            </>
                        ) : (
                            <>
                    ...
                    <p className="text-indigo-100 text-lg max-w-md">
                        {isNewStudent
                            ? "Your personalised learning journey starts here. Pick a lesson to begin!"
                            : totalCompleted > 0
                ...
                <div className="relative z-10">
                    {inProgressLesson ? (
                        ...
                        <Link href="/student/lessons">
                            <Button className="h-14 px-8 rounded-full bg-white text-indigo-600 hover:bg-indigo-50 font-bold shadow-lg border-0 text-base transition-transform hover:scale-105 active:scale-95">
                                <Play className="mr-2 h-5 w-5 fill-current" /> {isNewStudent ? "Start My First Lesson" : "Start Learning"}
                            </Button>
                        </Link>
                    )}
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="flex items-center justify-between">
                        <h2 className="text-2xl font-bold text-slate-800">
                            {isNewStudent ? "Recommended For You" : "Your Curriculum"}
                        </h2>
                    ...
                    <div className="space-y-4">
                        {/* Show in-progress + recent lessons if they exist, otherwise show suggested */}
                        {recentLessons.length > 0 ? (
                            recentLessons.slice(0, 3).map((lesson) => (
                                <LessonCard
                                    ...
                                    subjectColor={lesson.subject === "Mathematics" ? "bg-blue-500" : lesson.subject === "English" ? "bg-pink-500" : "bg-orange-500"}
                                    icon={lesson.subject === "Mathematics" ? <Calculator className="h-6 w-6 text-white" /> : lesson.subject === "English" ? <BookOpen className="h-6 w-6 text-white" /> : <Brain className="h-6 w-6 text-white" />}
                                />
                            ))
                        ) : suggested.length > 0 ? (
                            suggested.map((lesson) => (
                                <LessonCard
                                    ...
                                    subject={lesson.subject}
                                    title={lesson.title}
                                    description={lesson.description ?? "Start this lesson"}
                                    progress={0}
                                    subjectColor={lesson.subjectColor === "blue" ? "bg-blue-500" : lesson.subjectColor === "pink" ? "bg-pink-500" : "bg-orange-500"}
                                    icon={lesson.subject === "Mathematics" ? <Calculator className="h-6 w-6 text-white" /> : lesson.subject === "English" ? <BookOpen className="h-6 w-6 text-white" /> : <Brain className="h-6 w-6 text-white" />}
                                />
                            ))
                        ) : (
                            <div className="text-center p-8 bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-slate-500">
                                No lessons available. Contact your administrator.
                            </div>
                        )}
                    </div>
                </div>

                {/* Sidebar — Games & Challenges */}
                <div className="space-y-8">
                    <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-6">
                        ...
                        <div className="space-y-4">
                            <GameRow title="Math Blaster" category="Arithmetic" color="bg-indigo-100 text-indigo-600" />
                            <GameRow title="Word Wizard" category="Spelling" color="bg-purple-100 text-purple-600" />
                            <GameRow title="History Hero" category="Trivia" color="bg-amber-100 text-amber-600" />
                        </div>
                    ...
                    <div className="bg-slate-900 rounded-3xl p-6 text-white relative overflow-hidden">
                        ...
                        <p className="text-slate-300 text-sm mb-4 relative z-10">Complete 3 lessons to earn the &quot;Number Cruncher&quot; badge.</p>
...
function LessonCard({ lessonId, subject, title, description, progress, subjectColor, icon }: {
    lessonId: string; subject: string; title: string; description: string; progress: number; subjectColor: string; icon: React.ReactNode
}) {
    return (
        <Link href={`/lesson/${lessonId}`}>
            <div className="group bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md hover:border-violet-200 transition-all cursor-pointer flex items-center gap-5 relative overflow-hidden">
                <div className="absolute bottom-0 left-0 h-1 w-full bg-slate-100">
                    <div className={`h-full ${subjectColor}`} style={{ width: `${progress}%` }} />
                </div>
                <div className={`h-16 w-16 rounded-xl flex items-center justify-center shrink-0 shadow-sm ${subjectColor} transition-transform group-hover:scale-110 duration-300`}>
                    {icon}
                </div>
                ...
                <div className="hidden sm:flex flex-col items-end gap-1 text-slate-400">
                    ...
                    <Button size="icon" variant="ghost" className="rounded-full text-slate-300 group-hover:text-violet-600 group-hover:bg-violet-50 transition-all" aria-label="Start lesson">
...
function GameRow({ title, category, color }: { title: string; category: string; color: string }) {
    return (
        <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer group">
            ...
            <Button size="icon" variant="ghost" className="h-8 w-8 text-slate-300 group-hover:text-slate-600" aria-label={`Play ${title}`}>
